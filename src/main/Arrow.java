...
public class Arrow {
  ...
  /**
   * Draws line with arrow heads at either end.  Location of the arrowheads
   * along the line is controlled by the place parameter (for example, if
   * place = 0.45, arrowheads would be placed at spots 45% away from their
   * respective ends)
   *
   * @param g The graphics world to draw in
   * @param x1-y2 Coords of the line
   * @param start Arrowhead at start
   * @param end   Arrowhead at end
   * @param place A double [0-1] indicating the line length percentage (from start)
   *    at which the arrow tip will be placed.
   */
  ...
  public static void drawArrow(Graphics g, int x1, int y1, int x2, int y2) {
    drawArrow(g, x1, y1, x2, y2, false, true, (float)1);
  }
}
